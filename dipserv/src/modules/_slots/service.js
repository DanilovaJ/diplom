...
const SlotsRepository = require('../_slots/repository');
...
const service = {
	...
	getByDateToStudent: async (place_id, date) => {
		...
		const data = await repository.getSlotsByWeekday(place_id, WEEKDAYS[dateObject.getDay()]);
		repository.disconnect();
		const filteredData = new Date(date).toLocaleDateString() === new Date().toLocaleDateString() 
